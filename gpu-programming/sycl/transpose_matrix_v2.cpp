...
auto transposeKernel(sycl::handler &cgh, const float *in, float *out, int width,
  ...
  return [=](sycl::nd_item<2> item) {
    int a_tile_index = item.get_group(1) * tile_dim;
    int b_tile_index = item.get_group(0) * tile_dim;
    int x_local_index = item.get_local_id(1);
    int y_local_index = item.get_local_id(0);
    int in_index =
        (a_tile_index + y_local_index) * width + (b_tile_index + x_local_index);
    int out_index =
        (b_tile_index + y_local_index) * width + (a_tile_index + x_local_index);
